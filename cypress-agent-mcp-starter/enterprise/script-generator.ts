...
function scanPageObjects(repoRoot: string): PageObjectIndexEntry[] {
  const pagesDir = path.join(repoRoot, 'cypress', 'support', 'pages');
  ...
  return fs.readdirSync(pagesDir)
    ...
    .map((file) => {
      ...
      return {
        ...
        importPath: `../../support/pages/${file.replace(/\.ts$/, '')}`,
...
function renderSecretHelpers(provider: SecretProvider): string {
  if (provider === 'vault') {
    return [
      ...
      "  return Cypress.env(key) || `vault://qa/${key}`;",
      '}',
    ].join('\n');
  }
  ...
  return [
    ...
    "  return Cypress.env(key) || `aws-secrets-manager://qa/${key}`;",
    '}',
  ].join('\n');
}

function renderCypressScript(request: ScriptGenerationRequest, matches: PageObjectMethodMatch[], secretPlaceholders: string[]): string {
  const imports = ["/// <reference types=\"cypress\" />"];
  ...
  matches.forEach((match, index) => {
    ...
    objectConstruction.push(`    const ${variableName} = new ${match.className}();`);
    if (match.methodNames[0]) {
      reusableSteps.push(`    ${variableName}.${match.methodNames[0]}();`);
    }
  });
  ...
  return [
    ...
    "  it('executes the generated enterprise flow', () => {",
    ...objectConstruction,
    ...secretAssignments,
    ...reusableSteps,
...
function renderCSharpScript(request: ScriptGenerationRequest, matches: PageObjectMethodMatch[], secretPlaceholders: string[]): string {
  ...
  return [
    ...usingLines,
    '',
    'namespace Generated.CypressEnterprise;',
    '',
...
export function generateEnterpriseScript(repoRoot: string, request: ScriptGenerationRequest): GeneratedScriptArtifact {
  ...
  if (request.framework === 'typescript-cypress') {
    return {
      framework: request.framework,
      fileName: `${request.testName.toLowerCase().replace(/[^a-z0-9]+/g, '-')}.cy.ts`,
      content: renderCypressScript(request, matches, secretPlaceholders),
...
export function reviewGeneratedCode(content: string): ReviewBotFinding[] {
  ...
  lines.forEach((line, index) => {
    if (/cy\.wait\(\d+\)|waitForTimeout|sleep\(/.test(line)) {
      findings.push({
        severity: 'error',
        rule: 'no-hard-waits',
        message: 'Replace hard waits with Cypress retries, route waits, or assertion-based synchronization.',
        line: index + 1,
      });
    }
    ...
    if (/password\s*=\s*["'][^"']+["']|api[_-]?key\s*[:=]\s*["'][^"']+["']/i.test(line)) {
      findings.push({
        ...
        message: 'Credentials must come from Vault, AWS Secrets Manager, or Cypress env placeholders.',
        line: index + 1,
      });
    }
  });
