...
This document tracks how the Playwright starter implements the enterprise requirements requested for AI-assisted QA workflows.

## Status

Before this pass, the repository had conventions and documentation for page objects, PR hygiene, and flaky-test practices, but it did not implement the enterprise governance workflows themselves.

The implementation now lives under [`enterprise/`](../enterprise) and is verified by [`tests/api/enterprise-requirements.spec.ts`](../tests/api/enterprise-requirements.spec.ts).
...
- Framework support:
  - TypeScript Playwright and C# renderers in [`enterprise/script-generator.ts`](../enterprise/script-generator.ts)
...
- AUT log correlation across Datadog, Splunk, CloudWatch, and Playwright-style evidence:
...
- Accessibility finding analysis with fix recommendations and generated Playwright assertions:
